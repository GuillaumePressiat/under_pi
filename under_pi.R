...
alls <- 3:nmax %>% purrr::map(function(x)poly(x)$poly) %>% bind_rows()  %>% 
  mutate(estimate = sqrt(2 - 2 * cos(pi / n)) * n)
...
p <- ggplot() +
  ...
  #geom_line(aes(x = long, y = lat, group = group), col = 'grey30', lwd = 1.7) + 
  labs(title = 'Segments number : {frame_time}') + 
  transition_time(n)

image <- animate(p, nframes = 300, fps = 6)
anim_save('C:/under_pi.gif', image)
